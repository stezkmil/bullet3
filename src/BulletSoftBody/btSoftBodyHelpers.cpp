...
/* Reverse method for creating the .inl file from an existing btSoftBody is in btSoftBodyHelpers::writeTetGenData */
...
static std::string formatTetGenScalar(btScalar value)
{
	std::ostringstream stream;
	stream.imbue(std::locale::classic());
	stream << std::setprecision(std::numeric_limits<btScalar>::max_digits10) << value;
	return stream.str();
}

static void writeTetGenStringLine(std::ostream& stream, const std::string& line)
{
	stream << "\"" << line << "\\n\"";
}

void btSoftBodyHelpers::writeTetGenData(const char* filename, const btSoftBody* psb)
{
	std::ofstream fs;
	fs.open(filename);
	btAssert(fs);
	...
	fs << "// Generated by btSoftBodyHelpers::writeTetGenData\n";
	fs << "static const char* getNodes() { return(\n";
	writeTetGenStringLine(fs, std::to_string(psb->m_nodes.size()) + " 3 0 0");
	fs << "\n";

	for (int i = 0; i < psb->m_nodes.size(); ++i)
	{
		const btVector3& x = psb->m_nodes[i].m_x;
		writeTetGenStringLine(fs,
							  std::to_string(i) + " " +
								  formatTetGenScalar(x.getX()) + " " +
								  formatTetGenScalar(x.getY()) + " " +
								  formatTetGenScalar(x.getZ()));
		fs << "\n";
	}
	fs << "); }\n\n";

	fs << "static const char* getElements() { return(\n";
	writeTetGenStringLine(fs, std::to_string(psb->m_tetras.size()) + " 4 0");
	fs << "\n";

	const btSoftBody::Node* firstNode = psb->m_nodes.size() ? &psb->m_nodes[0] : 0;
	for (int i = 0; i < psb->m_tetras.size(); ++i)
	{
		const btSoftBody::Tetra& tetra = psb->m_tetras[i];
		writeTetGenStringLine(fs,
							  std::to_string(i) + " " +
								  std::to_string(static_cast<int>(tetra.m_n[0] - firstNode)) + " " +
								  std::to_string(static_cast<int>(tetra.m_n[1] - firstNode)) + " " +
								  std::to_string(static_cast<int>(tetra.m_n[2] - firstNode)) + " " +
								  std::to_string(static_cast<int>(tetra.m_n[3] - firstNode)));
		fs << "\n";
	}
	fs << "); }\n";
	fs.close();
