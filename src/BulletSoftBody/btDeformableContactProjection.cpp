...
btScalar btDeformableContactProjection::update(btCollisionObject** deformableBodies, int numDeformableBodies, const btContactSolverInfo& infoGlobal)
{
	...
	for (int i = 0; i < numDeformableBodies; ++i)
	{
		for (int j = 0; j < m_softBodies.size(); ++j)
		{
			...
			{
				btDeformableDeformableContactConstraint& constraint = m_nodeNodeConstraints[j][k];
				btScalar localResidualSquare = constraint.solveConstraint(infoGlobal);
				residualSquare = btMax(residualSquare, localResidualSquare);
			}
...
void btDeformableContactProjection::setConstraints(const btContactSolverInfo& infoGlobal)
{
	...
	for (int i = 0; i < m_softBodies.size(); ++i)
	{
		...
		for (int j = 0; j < psb->m_nodeNodeContacts.size(); ++j)
		{
			...
			btDeformableDeformableContactConstraint constraint(contact, infoGlobal);
...
void btDeformableContactProjection::setProjection()
{
	...
	for (int i = 0; i < m_softBodies.size(); ++i)
	{
		...
		{
			int index = m_nodeNodeConstraints[i][j].m_node->index;
			m_nodeNodeConstraints[i][j].m_node->m_constrained = true;
			if (m_nodeNodeConstraints[i][j].m_binding)
			{
				if (m_nodeNodeConstraints[i][j].m_static)
				{
					if (m_projectionsDict.find(index) == NULL)
					{
						m_projectionsDict.insert(index, units);
				...
				else
				{
					...
					{
						btAlignedObjectArray<btVector3> projections;
						projections.push_back(m_nodeNodeConstraints[i][j].m_normal);
						m_projectionsDict.insert(index, projections);
					}
					else
					{
...
void btDeformableContactProjection::setLagrangeMultiplier()
{
	for (int i = 0; i < m_softBodies.size(); ++i)
	{
		...
		for (int j = 0; j < m_nodeNodeConstraints[i].size(); ++j)
		{
			...
			{
				lm.m_num_constraints = 1;
				lm.m_dirs[0] = m_nodeNodeConstraints[i][j].m_normal;
			}
			m_lagrangeMultipliers.push_back(lm);
		}
...
void btDeformableContactProjection::applyDynamicFriction(TVStack& f)
{
	for (int i = 0; i < m_softBodies.size(); ++i)
	{
		...
		for (int j = 0; j < m_nodeNodeConstraints[i].size(); ++j)
		{
			const btDeformableDeformableContactConstraint& constraint = m_nodeNodeConstraints[i][j];
			const btSoftBody::Node* node = constraint.m_node;
			if (node->m_im != 0)
			{
				int index = node->index;
