	...
	for (int i = 0; i < m_softBodies.size(); ++i)
	{
		...
		else
		{
			for (int j = 0; j < psb->m_nodes.size(); ++j)
			{
				...
				if (j == 0)
					fprintf(stderr, "node %d pos %f %f %f vel %f %f %f force %f %f %f oldrepo\n", j, psb->m_nodes[j].m_x.x(), psb->m_nodes[j].m_x.y(), psb->m_nodes[j].m_x.z(), psb->m_nodes[j].m_v.x(), psb->m_nodes[j].m_v.y(), psb->m_nodes[j].m_v.z(), force[counter - 1].x(), force[counter - 1].y(), force[counter - 1].z());
			}
		}
	}
	if (setZero)
