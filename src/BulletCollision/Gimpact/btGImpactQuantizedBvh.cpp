...
// OUTDATED findOnlyFirstTriPair behaviour
static void _find_quantized_collision_pairs_recursive_ser(
	...
	const BT_BOX_BOX_TRANSFORM_CACHE& trans_cache_1to0,
	int node0, int node1, bool complete_primitive_tests, btFindOnlyFirstPairEnum findOnlyFirstTriPair)
{
	...
	else if (findOnlyFirstTriPair == btFindOnlyFirstPairEnum::DISABLED && boxset0->isLeafNode(node0) && boxset1->isLeafNode(node1))
	{
		// Leaf vs leaf test is not done now (except for the findOnlyFirstTriPair mode), but deferred to be done in the parallelized for loop in collide_sat_triangles.
	...
	if (boxset0->isLeafNode(node0))
	{
		...
		else
		{
			...
			_find_quantized_collision_pairs_recursive_ser(boxset0, boxset1, collision_pairs, trans_cache_1to0, node0, boxset1->getLeftNode(node1), false, findOnlyFirstTriPair);
			//collide right recursive
			_find_quantized_collision_pairs_recursive_ser(boxset0, boxset1, collision_pairs, trans_cache_1to0, node0, boxset1->getRightNode(node1), false, findOnlyFirstTriPair);
		}
	}
	else
	{
		...
		{
			//collide left recursive
			_find_quantized_collision_pairs_recursive_ser(boxset0, boxset1, collision_pairs, trans_cache_1to0, boxset0->getLeftNode(node0), node1, false, findOnlyFirstTriPair);
			_find_quantized_collision_pairs_recursive_ser(boxset0, boxset1, collision_pairs, trans_cache_1to0, boxset0->getRightNode(node0), node1, false, findOnlyFirstTriPair);
		}
		else
		{
			//collide left0 left1
			_find_quantized_collision_pairs_recursive_ser(boxset0, boxset1, collision_pairs, trans_cache_1to0, boxset0->getLeftNode(node0), boxset1->getLeftNode(node1), false, findOnlyFirstTriPair);

			//collide left0 right1
			_find_quantized_collision_pairs_recursive_ser(boxset0, boxset1, collision_pairs, trans_cache_1to0, boxset0->getLeftNode(node0), boxset1->getRightNode(node1), false, findOnlyFirstTriPair);

			//collide right0 left1
			_find_quantized_collision_pairs_recursive_ser(boxset0, boxset1, collision_pairs, trans_cache_1to0, boxset0->getRightNode(node0), boxset1->getLeftNode(node1), false, findOnlyFirstTriPair);

			//collide right0 right1
			_find_quantized_collision_pairs_recursive_ser(boxset0, boxset1, collision_pairs, trans_cache_1to0, boxset0->getRightNode(node0), boxset1->getRightNode(node1), false, findOnlyFirstTriPair);
	...
	int node0, int node1, bool complete_primitive_tests, btFindOnlyFirstPairEnum findOnlyFirstTriPair)
{
	...
	tbb::parallel_for_each(
		...
		[findOnlyFirstTriPair, boxset0, boxset1, trans_cache_1to0, collision_pairs, &collision_pairs_mutex](ElemType& elem, tbb::feeder<ElemType>& feeder)
		{
...
static void _find_quantized_collision_pairs_recursive_par(GroupedParams& groupedParams, int node0, int node1, int level, bool complete_primitive_tests)
{
	...
	else if (groupedParams.findOnlyFirstTriPair == btFindOnlyFirstPairEnum::DISABLED && groupedParams.boxset0->isLeafNode(node0) && groupedParams.boxset1->isLeafNode(node1))
	{
...
#endif
	if (grpParams.previouslyConsumedTime <= 100 || findOnlyFirstTriPair == btFindOnlyFirstPairEnum::PENETRATING || findOnlyFirstTriPair == btFindOnlyFirstPairEnum::TOUCHING || disableParallel)
