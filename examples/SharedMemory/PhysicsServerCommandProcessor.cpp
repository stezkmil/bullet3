		...
		for (int c = 0; c < psb->m_faceRigidContacts.size(); c++)
		{
			...
			else
			{
				...
				btVector3 normalForce = btVector3(btScalar(pt->m_contactNormalOnBInWS[0]),
												  ...
												  btScalar(pt->m_contactNormalOnBInWS[2])) *
										pt->m_normalForce;
				...
				if (psb)
				{
					...
					if (deformWorld)
					{
						...
						if (face_id >= 0 && face_id < psb->m_faces.size())
						{
							...
							btDeformableMousePickingForce* mouse_force = new btDeformableMousePickingForce(100, 0, &f, nullptr, nullptr, btTransform(btTransform::getIdentity().getBasis(), pickPos), m_data->m_maxPickingForce);
